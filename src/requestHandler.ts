...
import {
  createMockResponseFromXHR,
  getCookie,
  getFullURL,
  getXHRCompatibleBody,
  serializeParams,
} from "./utils";
...
export async function handleRequest<T>(
  ...
  switch (finalConfig.responseType) {
    case "blob":
      responseData = await finalResponse.blob();
      break;
    case "arraybuffer":
      responseData = await finalResponse.arrayBuffer();
      break;
    case "text":
  ...
  if (!validateStatus(finalResponse.status)) {
    throw new Error(`Request failed with status code ${finalResponse.status}`);
  }
...
async function handleProgressRequest<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = async () => {
      ...
      resolve({
        data: await finalResponse.json(), // Always parse as JSON
        status: finalResponse.status,
        statusText: finalResponse.statusText,
        headers: finalResponse.headers,
        config: finalConfig,
